public class Array{
    ...
    public String maximaTemperatura(){
        ...
        String max = this.vetorTemperaturas[0];
        for(int i = 0; i < this.vetorTemperaturas.length; i++){
            if(this.vetorTemperaturas[i] != null){
                String aux=this.vetorTemperaturas[i];
                float maxConvertido = Float.parseFloat(max);
                float auxConvertido = Float.parseFloat(aux);
                if(maxConvertido<auxConvertido){
                    max = aux;
                }
            }
        }
    ...
    public String minimaTemperatura(){
        ordenarCrescentemente();
        String min = this.vetorTemperaturas[0];
        for(int i = 0; i < this.vetorTemperaturas.length; i++){
            if(this.vetorTemperaturas[i] != null){
                String aux=this.vetorTemperaturas[i];
                float minConvertido = Float.parseFloat(min);
                float auxConvertido = Float.parseFloat(aux);
                if(minConvertido>auxConvertido){
                    min = aux;
                }
            }
        }
