...
int main (int argc, char *argv[]) 
{
  ...
  int pedaco;        // tamanho do bloco de dados do vetor
  
  pedaco = N/100;   
    
  sa = 3;
  
  //omp_set_num_threads(10);  // define a qtde de threads a serem criadas
...
#pragma omp parallel 
{
   printf("\nQtde threads: %d", omp_get_num_threads());
   #pragma omp for schedule(dynamic, pedaco)	
  // laco para calculo com os vetores - sapxy
  for (i=0; i<N; i++) 
     Y[i] = sa * X[i] + Y[i]; 
}  // final do bloco paralelo
